...
async def process_pdf_image(image_path, query):
    ...
    try:
        ...
        # Use JsonOutputParser to parse the content
        parser = JsonOutputParser()
        parsed_content = parser.parse(result.content)
        
        return {"image": image_path, "analysis": parsed_content}
    except Exception as e:
...
async def analyze_pdf_images(query):
    ...
    retrieved_images = search_faiss(query, k=3)
...
async def summarize_responses(query, responses):
    ...
    formatted_responses = "\n\n".join(
        [f"- {res['image']}\nSummary: {res['analysis']['Summary']}\nKey Figures: {res['analysis']['Key Figures']}\nRisks: {res['analysis']['Risks or Notes']}" for res in responses]
    )
    ...
    try:
        ...
        summary_content = summary.content  # Assuming 'content' is the correct attribute
